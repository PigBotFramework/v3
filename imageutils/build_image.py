...
class BuildImage:
    ...
    def save_jpg(self) -> BytesIO:
        output = '{0}.jpg'.format(time.time())
        image = self.image.convert("RGB")
        image.save('/pbf/resources/createimg/'+output, format="jpeg")
        return output

    def save_png(self) -> BytesIO:
        output = '{0}.png'.format(time.time())
        image = self.image.convert("RGBA")
        image.save('/pbf/resources/createimg/'+output, format="png")
        return output